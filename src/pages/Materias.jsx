...
import { progressoPorMateria, proximoSubtopico } from '../lib/stats'

export default function Materias() {
  const { subtopicoStatus } = useStore()
  const progresso = useMemo(() => progressoPorMateria(subtopicoStatus), [subtopicoStatus])
    ...
    <>
      ...
      <div className="grade grade-2 mt-2">
        {MATERIAS.map((m) => {
          ...
          const proximo = proximoSubtopico(m.id, subtopicoStatus)
          ...
          return (
            <div key={m.id} className="card" style={{ borderTop: `4px solid ${m.cor}` }}>
              ...
              <div className="linha linha-quebra" style={{ gap: '.4rem', marginBottom: '.7rem' }}>
                ...
                <span className="etiqueta etiqueta-cinza">{p.total} subtópicos</span>
              ...
              <div className="linha linha-quebra">
                <Link to={`/materias/${m.id}`} className="btn btn-sm">Ver conteúdo</Link>
                {proximo && (
                  <Link to={`/subtopico/${proximo.id}`} className="btn btn-neutro btn-sm">Continuar de onde parei</Link>
                )}
              </div>
            </div>
          )
